...
# --- APP CONFIG ---
st.set_page_config(page_title="SMC-ICT PRO v3.4", layout="centered")
st.title("SMC-ICT PRO v3.4")
...
def log_trade(signal_type, entry_p, sl_p, tp_p, reason):
    ...
    pnl_val = 20.0 if signal_type == "BUY" else 20.0
    journal.append({
        "timestamp": now_str, 
        "type": signal_type, 
...
session_high = data['High'].iloc[-20:].max() + manual_offset
session_low = data['Low'].iloc[-20:].min() + manual_offset
